...
// Generate HTML team profile
const generate = (teamArr) => {
    
    // Functions to create new employee card array by role
    ...
    // HTML array by role for employees
...
const generateHTML = (teamArr) => {
    fs.writeFile('./dist/index.html', generate(teamArr), function(err, result) {
        (err) ? console.log('error', err): console.log(`\r\nAdded to dist folder successfully!`.underline.green);
        }
    );
};
